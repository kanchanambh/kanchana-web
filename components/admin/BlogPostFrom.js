...
function BlogPostFrom({
  ...
  imgDeleted,
  handleSubmitBodyImg,
  handleFileChangeBodyImg,
  selectedFilesBodyImg,
  setSelectedFilesBodyImg,
  imageBodyImgURL,
  setImageBodyImgURL,
  uploadedImageURLS,
  ...
  return (
    <div className="pt-5  w-[80%] ">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <textarea
            ...
            className="flex-1 bg-slate-700 w-full h-[250px] font-light mb-0 focus:outline-none pl-1 p-[2px] rounded-md"
            name="description"
            id=""
              ...
              productInfo.images.map((seledted, i) => (
                <div className="w-[200px] h-[150px]" key={i}>
                  <img
                    ...
                    src={seledted}
                    onClick={(e) => imgDeleted(e, seledted, "images")}
                  />
                </div>
              ))}
          </ReactSortable>
      ...
      <div className="pt-10">
        <h2>Upload Files</h2>
        <div className="flex pl-10  space-x-10">
          {imageBodyImgURL.length > 0 &&
            imageBodyImgURL.map((seledted, i) => (
              <div className="w-[120px] h-[100px]" key={i}>
                <img className="w-full h-full object-cover" src={seledted} />
              </div>
            ))}
        </div>
        <form onSubmit={handleSubmitBodyImg}>
          <input type="file" multiple onChange={handleFileChangeBodyImg} />
          <button type="submit">Upload</button>
          <div>
            {uploadedImageURLS.length > 0 &&
              uploadedImageURLS.map((image, i) => (
                <div key={i}>
                  <p className="pt-10">{image}</p>
                  <div className="w-[200px] h-[150px]">
                    <img
                      className="w-full h-full object-cover"
                      src={image}
                      onClick={(e) => imgDeleted(e, image, "images")}
                    />
                  </div>
                </div>
              ))}
          ...
          <div>
            {productInfo.bodyimages.length > 0 &&
              productInfo.bodyimages.map((seledted, i) => (
                <div className="w-[200px] h-[150px]" key={i}>
                  <img
                    className="w-full h-full object-cover"
                    src={seledted}
                    onClick={(e) => imgDeleted(e, seledted, "bodyimages")}
                  />
                </div>
              ))}
          </div>
